...
public class UserProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Date birthDate = new Date(getDateFromString(request.getParameter("dob")));
        String address = request.getParameter("address");
        User u = new User(name_user, id_user, phone, address, birthDate);
        UserDAO userDao = new UserDAO();
        // Insert information to database
        userDao.EditProfile(u);
        User u1 = userDao.getUserByid(id_user);
        session.setAttribute("user", u1);
        response.sendRedirect("UserProfile?userid="+id_user);
