...
public class ListQuizServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int idLesson = 0;
        if (request.getAttribute("idLessonFromSerlet") != null) {
            idLesson = (int) request.getAttribute("idLessonFromSerlet");
        } else {
            idLesson = Integer.parseInt(request.getParameter("idLesson"));
        }
        SubjectDAO daoSubject = new SubjectDAO();
        QuizDAO daoQuiz = new QuizDAO();
        ArrayList<Subject> infoSubject = daoSubject.getAllSubjects();
        ArrayList<Quiz> infoQuiz = daoQuiz.getAllQuizByIdLesson(idLesson);
        request.setAttribute("idLesson", idLesson);
        request.setAttribute("infoSubject", infoSubject);
        request.setAttribute("infoQuiz", infoQuiz);
